...
package screens {
	...
	public class InGame extends BaseScreen 
	{
		...
		override public function initialize():void
		{
			...
			bruja.x = -stage.stageWidth;
			bruja.y = stage.stageHeight * 0.5;		
			...
			// Resetea los patrones de paginas.
			pattern = 1;
		...
		private function onGameTick(event:Event):void
		{
			...
			switch(gameState)
			{
				...
				case GameConstants.GAME_STATE_IDLE:
					...
					if (bruja.x < stage.stageWidth * 0.5 * 0.5)
					{
						bruja.x += ((stage.stageWidth * 0.5 * 0.5 + 10) - bruja.x) * 0.05;
						bruja.y = stage.stageHeight * 0.5;
				...
				//	}
					...
					if (hitDiablo <= 0)	{
						...
						if (mana <= 0)
						{
							...
							if (bruja.y > stage.stageHeight * 0.5) bruja.rotation -= deg2rad(hitDiablo * 2);
				...
				case GameConstants.GAME_STATE_OVER:
					...
					if (bruja.y < stage.stageHeight + bruja.width)
					{
						playerSpeed -= playerSpeed * elapsed;
						bruja.y += stage.stageHeight * elapsed; 
					}
					else
					{
	 ...
	 */	
		...
		private function createItems():void
		{
			...
			{
				case 1:
					// Horizontal, crea un solo item, y cambia la posicion del patron al azar.
					if (Math.random() > 0.9)
					{
					...
					/*itemToTrack = new Item(GameConstants.ITEM_TYPE_1);
					this.addChild(itemToTrack);*/
					itemToTrack = itemFactory.getItem(GameConstants.ITEM_TYPE_1);
					this.addChild(itemToTrack);
					//	itemToTrack.itemType = Math.ceil(Math.random() * 1);
				...
				case 3:
					...
					// luego hace lo mismo hasta arriba.
					
					// Cambia la direccion del patron al rozar los bordes.
				...
				case 4:
					// Random, crea un N cantidad de items en pantalla.
					if (Math.random() > 0.3)
					{
						...
						{
							// Checkout item del pool y asigna el tipo.
							/*itemToTrack = new Item(GameConstants.ITEM_TYPE_1);
							this.addChild(itemToTrack); */
							...
							// Incrementa la posicion del siguinte item or un valor aleatorio.
				...
				case 10:
					...
					// Checkout item del pool y asigna el tipo.
					/*itemToTrack = new Item(GameConstants.ITEM_TYPE_MANA);
					this.addChild(itemToTrack); */
				...
				case 11:
					...
					// Checkout item del pool y asigna el tipo.
					/*itemToTrack = new Item(GameConstants.ITEM_TYPE_REFERENCIA);
					this.addChild(itemToTrack);*/ 
		...
		private function createDiablos(_type:int, _distance:Number):void
		{
			...
			// Nuevo diablo.
			var enemigo:Diablo = new Diablo(_type, _distance);
